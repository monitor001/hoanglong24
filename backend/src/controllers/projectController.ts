...
export const createProject = async (req: Request, res: Response) => {
  try {
    ...
    if (memberIds && Array.isArray(memberIds)) {
      // Filter out invalid memberIds and current user (to avoid duplicate)
      validatedMemberIds = memberIds.filter(id => 
        typeof id === 'string' && 
        id.length > 0 && 
        id !== req.user?.id
      );
    ...
    const project = await prisma.$transaction(async (prisma) => {
      ...
      const projectCode = generateProjectCode(name.trim());
 ...
 * @route GET /api/projects/:id/members/:memberId/permissions
 */
export const getProjectMemberPermissions = async (req: Request, res: Response) => {
  try {
    const { id, memberId } = req.params;
    ...
    // Check if user has permission to view member permissions
    if (req.user?.role !== 'ADMIN') {
      const userMembership = project.members.find((member: any) => member.userId === req.user?.id);
      ...
      if (!['PROJECT_MANAGER', 'BIM_MANAGER'].includes(userMembership.role)) {
        throw new ApiError(403, 'You do not have permission to view member permissions');
      }
    }
    ...
    console.error('Get project member permissions error:', error);
    if (error instanceof ApiError) {
      res.status(error.statusCode).json({ error: error.message });
    } else {
      res.status(500).json({ error: 'Internal server error' });
 ...
 * @route PUT /api/projects/:id/members/:memberId/permissions
 */
export const updateProjectMemberPermissions = async (req: Request, res: Response) => {
  try {
    const { id, memberId } = req.params;
    ...
    if (req.user?.role !== 'ADMIN') {
      ...
      if (!['PROJECT_MANAGER'].includes(userMembership.role)) {
        throw new ApiError(403, 'You do not have permission to update member permissions');
      }
    }
    ...
    const validResources = ['PROJECT', 'DOCUMENTS', 'TASKS', 'ISSUES', 'CALENDAR', 'COMMENTS', 'APPROVALS'];
    const validActions = ['VIEW', 'CREATE', 'EDIT', 'DELETE', 'APPROVE', 'SHARE', 'EXPORT'];
    
    for (const permission of permissions) {
      if (!validResources.includes(permission.resource)) {
        throw new ApiError(400, `Invalid resource: ${permission.resource}`);
      }
      if (!validActions.includes(permission.action)) {
        throw new ApiError(400, `Invalid action: ${permission.action}`);
      }
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const newPermissions = await Promise.all(
        permissions.map(async (permission) => {
          return await tx.projectPermission.create({
            data: {
              memberId,
              resource: permission.resource,
              action: permission.action,
              granted: permission.granted || false,
              grantedAt: permission.granted ? new Date() : null,
              grantedBy: permission.granted ? req.user?.id : null
            }
          });
        })
      );
    ...
    console.error('Update project member permissions error:', error);
    if (error instanceof ApiError) {
      res.status(error.statusCode).json({ error: error.message });
    } else {
      res.status(500).json({ error: 'Internal server error' });
 ...
 * @route GET /api/projects/roles/:role/permissions
 */
export const getDefaultRolePermissions = async (req: Request, res: Response) => {
  try {
    const { role } = req.params;
    
    const validRoles = ['ADMIN', 'PROJECT_MANAGER', 'BIM_MANAGER', 'CONTRIBUTOR', 'VIEWER', 'USER'];
    ...
    const defaultPermissions: { [key: string]: any[] } = {
      ADMIN: [
        { resource: 'PROJECT', action: 'VIEW', granted: true },
        { resource: 'PROJECT', action: 'CREATE', granted: true },
        { resource: 'PROJECT', action: 'EDIT', granted: true },
        { resource: 'PROJECT', action: 'DELETE', granted: true },
        ...
        { resource: 'DOCUMENTS', action: 'DELETE', granted: true },
        { resource: 'DOCUMENTS', action: 'APPROVE', granted: true },
        { resource: 'DOCUMENTS', action: 'SHARE', granted: true },
        { resource: 'DOCUMENTS', action: 'EXPORT', granted: true },
        { resource: 'TASKS', action: 'VIEW', granted: true },
        ...
        { resource: 'ISSUES', action: 'VIEW', granted: true },
        { resource: 'ISSUES', action: 'CREATE', granted: true },
        { resource: 'ISSUES', action: 'EDIT', granted: true },
        { resource: 'ISSUES', action: 'DELETE', granted: true },
        { resource: 'ISSUES', action: 'APPROVE', granted: true },
        { resource: 'CALENDAR', action: 'VIEW', granted: true },
        { resource: 'CALENDAR', action: 'CREATE', granted: true },
        { resource: 'CALENDAR', action: 'EDIT', granted: true },
        { resource: 'CALENDAR', action: 'DELETE', granted: true },
        { resource: 'COMMENTS', action: 'VIEW', granted: true },
        { resource: 'COMMENTS', action: 'CREATE', granted: true },
        { resource: 'COMMENTS', action: 'EDIT', granted: true },
        { resource: 'COMMENTS', action: 'DELETE', granted: true },
        { resource: 'APPROVALS', action: 'VIEW', granted: true },
        { resource: 'APPROVALS', action: 'CREATE', granted: true },
        { resource: 'APPROVALS', action: 'EDIT', granted: true },
        { resource: 'APPROVALS', action: 'DELETE', granted: true },
        { resource: 'APPROVALS', action: 'APPROVE', granted: true }
      ],
      PROJECT_MANAGER: [
        { resource: 'PROJECT', action: 'VIEW', granted: true },
        { resource: 'PROJECT', action: 'CREATE', granted: true },
      ...
      BIM_MANAGER: [
        { resource: 'PROJECT', action: 'VIEW', granted: true },
        { resource: 'PROJECT', action: 'CREATE', granted: false },
        { resource: 'PROJECT', action: 'EDIT', granted: false },
        { resource: 'PROJECT', action: 'DELETE', granted: false },
        { resource: 'DOCUMENTS', action: 'VIEW', granted: true },
        { resource: 'DOCUMENTS', action: 'CREATE', granted: true },
        { resource: 'DOCUMENTS', action: 'EDIT', granted: true },
        ...
        { resource: 'TASKS', action: 'CREATE', granted: true },
        { resource: 'TASKS', action: 'EDIT', granted: true },
        { resource: 'TASKS', action: 'DELETE', granted: false },
        { resource: 'TASKS', action: 'APPROVE', granted: true },
        { resource: 'ISSUES', action: 'VIEW', granted: true },
        { resource: 'ISSUES', action: 'CREATE', granted: true },
        { resource: 'ISSUES', action: 'EDIT', granted: true },
        { resource: 'ISSUES', action: 'DELETE', granted: false },
        { resource: 'ISSUES', action: 'APPROVE', granted: true },
        { resource: 'CALENDAR', action: 'VIEW', granted: true },
        { resource: 'CALENDAR', action: 'CREATE', granted: true },
        { resource: 'CALENDAR', action: 'EDIT', granted: true },
        { resource: 'CALENDAR', action: 'DELETE', granted: false },
        { resource: 'COMMENTS', action: 'VIEW', granted: true },
        { resource: 'COMMENTS', action: 'CREATE', granted: true },
        { resource: 'COMMENTS', action: 'EDIT', granted: true },
        { resource: 'COMMENTS', action: 'DELETE', granted: false },
        { resource: 'APPROVALS', action: 'VIEW', granted: true },
        { resource: 'APPROVALS', action: 'CREATE', granted: true },
        { resource: 'APPROVALS', action: 'EDIT', granted: true },
        { resource: 'APPROVALS', action: 'DELETE', granted: false },
        { resource: 'APPROVALS', action: 'APPROVE', granted: true }
      ],
      CONTRIBUTOR: [
        { resource: 'PROJECT', action: 'VIEW', granted: true },
        ...
        { resource: 'DOCUMENTS', action: 'CREATE', granted: true },
        { resource: 'DOCUMENTS', action: 'EDIT', granted: true },
        { resource: 'DOCUMENTS', action: 'DELETE', granted: false },
        { resource: 'DOCUMENTS', action: 'APPROVE', granted: false },
        { resource: 'DOCUMENTS', action: 'SHARE', granted: true },
        { resource: 'DOCUMENTS', action: 'EXPORT', granted: true },
        { resource: 'TASKS', action: 'VIEW', granted: true },
        { resource: 'TASKS', action: 'CREATE', granted: true },
        { resource: 'TASKS', action: 'EDIT', granted: true },
        { resource: 'TASKS', action: 'DELETE', granted: false },
        { resource: 'TASKS', action: 'APPROVE', granted: false },
        { resource: 'ISSUES', action: 'VIEW', granted: true },
        { resource: 'ISSUES', action: 'CREATE', granted: true },
        { resource: 'ISSUES', action: 'EDIT', granted: true },
        { resource: 'ISSUES', action: 'DELETE', granted: false },
        { resource: 'ISSUES', action: 'APPROVE', granted: false },
        { resource: 'CALENDAR', action: 'VIEW', granted: true },
        { resource: 'CALENDAR', action: 'CREATE', granted: true },
        { resource: 'CALENDAR', action: 'EDIT', granted: true },
        { resource: 'CALENDAR', action: 'DELETE', granted: false },
        ...
        { resource: 'COMMENTS', action: 'DELETE', granted: false },
        { resource: 'APPROVALS', action: 'VIEW', granted: true },
        { resource: 'APPROVALS', action: 'CREATE', granted: false },
        { resource: 'APPROVALS', action: 'EDIT', granted: false },
        { resource: 'APPROVALS', action: 'DELETE', granted: false },
        { resource: 'APPROVALS', action: 'APPROVE', granted: false },
        { resource: 'USERS', action: 'CREATE', granted: false },
        { resource: 'USERS', action: 'EDIT', granted: false },
      ...
      VIEWER: [
        ...
        { resource: 'PROJECT', action: 'DELETE', granted: false },
        { resource: 'DOCUMENTS', action: 'VIEW', granted: true },
        { resource: 'DOCUMENTS', action: 'CREATE', granted: false },
        { resource: 'DOCUMENTS', action: 'EDIT', granted: false },
        { resource: 'DOCUMENTS', action: 'DELETE', granted: false },
        { resource: 'DOCUMENTS', action: 'APPROVE', granted: false },
        { resource: 'DOCUMENTS', action: 'SHARE', granted: false },
        { resource: 'DOCUMENTS', action: 'EXPORT', granted: true },
        { resource: 'TASKS', action: 'VIEW', granted: true },
        { resource: 'TASKS', action: 'CREATE', granted: false },
        { resource: 'TASKS', action: 'EDIT', granted: false },
        { resource: 'TASKS', action: 'DELETE', granted: false },
        { resource: 'TASKS', action: 'APPROVE', granted: false },
        { resource: 'ISSUES', action: 'VIEW', granted: true },
        { resource: 'ISSUES', action: 'CREATE', granted: false },
        { resource: 'ISSUES', action: 'EDIT', granted: false },
        { resource: 'ISSUES', action: 'DELETE', granted: false },
        { resource: 'ISSUES', action: 'APPROVE', granted: false },
        { resource: 'CALENDAR', action: 'VIEW', granted: true },
        { resource: 'CALENDAR', action: 'CREATE', granted: false },
        { resource: 'CALENDAR', action: 'EDIT', granted: false },
        { resource: 'CALENDAR', action: 'DELETE', granted: false },
        { resource: 'COMMENTS', action: 'VIEW', granted: true },
        { resource: 'COMMENTS', action: 'CREATE', granted: true },
        { resource: 'COMMENTS', action: 'EDIT', granted: false },
        { resource: 'COMMENTS', action: 'DELETE', granted: false },
        { resource: 'APPROVALS', action: 'VIEW', granted: true },
        { resource: 'APPROVALS', action: 'CREATE', granted: false },
        { resource: 'APPROVALS', action: 'EDIT', granted: false },
      ...
      USER: [
        ...
        { resource: 'DOCUMENTS', action: 'APPROVE', granted: false },
        { resource: 'DOCUMENTS', action: 'SHARE', granted: false },
        { resource: 'DOCUMENTS', action: 'EXPORT', granted: false },
        { resource: 'TASKS', action: 'VIEW', granted: true },
        { resource: 'TASKS', action: 'CREATE', granted: false },
        { resource: 'TASKS', action: 'EDIT', granted: false },
        { resource: 'TASKS', action: 'DELETE', granted: false },
        ...
        { resource: 'CALENDAR', action: 'DELETE', granted: false },
        { resource: 'COMMENTS', action: 'VIEW', granted: true },
        { resource: 'COMMENTS', action: 'CREATE', granted: false },
        { resource: 'COMMENTS', action: 'EDIT', granted: false },
        { resource: 'COMMENTS', action: 'DELETE', granted: false },
        { resource: 'APPROVALS', action: 'VIEW', granted: true },
        { resource: 'APPROVALS', action: 'CREATE', granted: false },
    ...
    console.error('Get default role permissions error:', error);
    if (error instanceof ApiError) {
      res.status(error.statusCode).json({ error: error.message });
    } else {
      res.status(500).json({ error: 'Internal server error' });
