  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      if (Array.isArray(response.data)) {
        ...
        projectsData = response.data.projects; // Backend trả về { projects: [...], pagination: {...} }
      } else if (response.data && Array.isArray(response.data.data)) {
        projectsData = response.data.data; // Nếu API trả về { data: [...] }
      } else {
...
const projectSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchProjects.fulfilled, (state, action) => {
      ...
      // Đảm bảo action.payload là array
      if (Array.isArray(action.payload)) {
        state.projects = action.payload;
      } else {
        console.error('fetchProjects.fulfilled - payload is not an array:', action.payload);
        state.projects = [];
      }
    });
