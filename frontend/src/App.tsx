...
// Private Route Component - Enhanced with automatic login redirect
const PrivateRoute: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const auth = useTypedSelector((state) => state.auth);
  const dispatch = useDispatch<AppDispatch>();
  
  // Note: User permissions are now loaded by PermissionLoader component
  
  // Check if user is authenticated - luôn yêu cầu đăng nhập mới
  if (!auth.token) {
...
// Dashboard Route Component - Simplified without permissions
...
// Admin Route Component - Simplified without permissions
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Setup all error handlers first
